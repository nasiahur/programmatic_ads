...
class ExclusionAdmin(admin.ModelAdmin):
    list_filter  = ['policy']
    list_display = ('value', 'policy', 'scan_adult', 'scan_categories', 'scan_custom', 'scan_file', 'scan_adblock', 'scan_privacy', 'scan_http')
...
#
#
class RuleAdsAdmin(admin.ModelAdmin):
    list_filter   = ['policy']
    list_display  = ('policy', 'block_ads', 'hide_ads', 'replace_with_image', 'apply_replace_heuristics')
admin.site.register(RuleAds, RuleAdsAdmin)

#
#
#
class RulePrivacyAdmin(admin.ModelAdmin):
    list_filter   = ['policy']
    list_display  = ('policy', 'protect_privacy', 'hide_trackers', 'replace_with_image', 'apply_replace_heuristics')
admin.site.register(RulePrivacy, RulePrivacyAdmin)
